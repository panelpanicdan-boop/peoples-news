// src/App.js — Option A footer + header profile dropdown for Settings
...
// --- Global ticker animation styles ---
const globalStyles = `
...
export default function App() {
  // Which tab is active (feed, upload, camera, live, account, settings, map)
  ...
  // --- Scroll-based bottom nav show/hide ---
  useEffect(() => {
    function onScroll() {
  ...
  useEffect(() => {
    if (tickerMode === "custom") return; // custom text is controlled by user
  ...
  function handleGoLive() {
    ...
    if (!user.verified) {
      alert("Verify your identity to go live (mock).");
      return;
    }
    if (ageDays < 7) {
      alert("Your account must be at least 7 days old to go live (mock).");
      return;
    }
    setIsLive(true);
    setUsersLive((v) => v + 1);
  ...
  const appStyle = darkMode ? styles.appDark : styles.app;
  ...
  return (
    <div style={appStyle}>
      ...
      <main style={styles.container}>
        ...
        {tab === "upload" && (
          <UploadTab posts={posts} setPosts={setPosts} />
        )}

        {tab === "camera" && (
          <CameraTab posts={posts} setPosts={setPosts} />
        )}
...
// --- HEADER with profile avatar dropdown + map button + live counter ---
function Header({
  user,
  usersLive,
  showTicker,
    ...
    <header style={styles.header}>
      <div style={styles.headerTop}>
        ...
        <div style={styles.headerRight}>
          ...
          {/* Map button & live users stacked */}
          <div style={styles.headerRightBottom}>
...
// --- BOTTOM NAV (Option A: Feed, Upload, Camera, Live, Account) ---
function BottomNav({ tab, setTab, showNav }) {
  return (
    ...
    >
      ...
      <button
        style={tab === "live" ? styles.navBtnActive : styles.navBtn}
        onClick={() => setTab("live")}
      >
        <FaBroadcastTower size={20} />
        <span style={styles.navLabel}>Live</span>
      </button>
